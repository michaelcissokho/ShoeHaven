...
class ShoeHavenApi {
    ...
    static async updateUser(password, firstname, lastname, email) {
        const response = await this.request(`users/${localStorage.getItem('username')}/update`, { password, firstname, lastname, email }, 'patch')
    ...
    static async createSale(listingId, seller) {
        const response = await this.request(`sales/new`, { listingId, seller, buyer: localStorage.getItem('username'), returned: false }, 'post')
